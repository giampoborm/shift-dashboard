...
- **A payslip can be WRONG.** `Payslip.vacationDisputed` marks a month payroll miscounted (8/2026: the manager counted 9 days away for an 8-day trip). Its figures stay authoritative for money and the entitlement balance — he was charged them — but `calibrateCharge` stops checking the model against it. Never "fix" the model to match a disputed slip.
...
- **The model's self-check is a WARNING, not a read-out.** `CalibrationWarning` renders nothing when the estimate agrees with the payslips, two dismissible lines when it doesn't (dismissal keyed to the specific discrepancy, so it returns if the numbers move — which is what makes "re-check on the next payslip" work by itself). Don't reintroduce an always-on status badge — the user asked for silence when things are fine, and **never phrase a mismatch as "your log is wrong"**: either side can be off (see the open question below).
