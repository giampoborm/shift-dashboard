 ...
 * and worked/sick (already resolved — a sick day is settled, not pending).
 * Deliberately NOT filtered to "on or after today": a
...
export function nextShiftFrom(shifts: Shift[], _today: Date): Shift | null {
  ...
  for (const s of shifts) {
    if (s.status === "swapped-out" || s.status === "worked" || s.status === "sick") continue;
